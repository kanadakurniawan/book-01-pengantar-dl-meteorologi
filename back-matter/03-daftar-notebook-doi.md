...
| `notebooks/ch-06-05_persiapan_data.ipynb` | 6 | Data BMKG/ERA5; format; QC; fitur; split | Persiapan data lengkap |
...
| `notebooks/ch-09-08_studi_kasus_curah_hujan_bmkg.ipynb` | 9 | Studi kasus hujan BMKG; klasifikasi + verifikasi | — |
...
2. **Lokal**: pastikan `pip install tensorflow numpy pandas matplotlib`
   (lihat `requirements` tiap notebook; Bab 8 butuh `requests` untuk skrip
   unduh, `xarray`/`netCDF4` opsional untuk Bab 6).
...
- **Versi TensorFlow** tercatat di metadata notebook (atau di Bab 1/5 saat
  instalasi). Snapshot lingkungan (mulai dari rilis) akan dirilis di Zenodo.
- **Data**: sample di repo + skrip unduh; snapshot data studi kasus di Zenodo
  (lihat "Daftar Dataset & Sumber").
...
| Dataset studi kasus (snapshot) | `10.5281/zenodo.0000000` | **Placeholder** — didaftarkan bersama DO data |
| Versi per-bab | — (tidak ada DOI per bab; sitasi stabil via DOI buku) | — |

> Prinsip: **1 buku = 1 DOI Zenodo** (konsep versi): v1.0 (Bab 1–5) →
> v1.1 (Bab 6–9) → v2.0 (lengkap). Saat DOI dibuat, placeholder diganti dan
> disalin ke field `bookDOI` di semua `master.md`.
