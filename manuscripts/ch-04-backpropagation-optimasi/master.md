...
1. **Forward *pass*:** `x → z¹ → a¹ (ReLU) → z² → pred → loss`. Nilai disimpan di tiap lapisan (dibutuhkan nanti).
...
Bab 2-4 memberi Anda satu siklus penuh: bentuk data → pelatihan → evaluasi. Dalam praktik, Anda akan mengulang siklus ini berkali-kali:
...
1. Turunkan secara intuitif: mengapa kita bergerak *berlawanan* arah gradien saat *gradient descent*?
2. Apa itu *vanishing gradient* dan bagaimana ReLU membantu?
3. Apa beda *learning rate*, *batch size*, dan *epochs* - dan bagaimana masing-masing memengaruhi pelatihan?
